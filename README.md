...
Como o contêiner roda com um usuário não-root, ele não tem permissão para usar portas abaixo de 1024. O Apache foi reconfigurado para escutar na porta 8080, seguindo a prática do "menor privilégio".
...
- Um ALB (Application Load Balancer) como ponto de entrada.

### Extensão para Implantação Contínua (CD)
...
2. Navegue para a pasta `terraform` e execute `terraform init`.
3. Execute `terraform apply` e confirme com `yes`.
4. Acesse o site através da `URL_do_Site_WordPress` exibida no output.
